...
pub fn chained_mempool_tx_sync() -> anyhow::Result<()> {
    ...
    let list_unspent = env.list_unspent()?;

    // Create second unconfirmed tx that spends the first.
    let utxo = list_unspent
        .0
        .into_iter()
        .find(|utxo| utxo.script_pubkey == tracked_addr.script_pubkey())
...
fn test_check_fee_calculation() -> anyhow::Result<()> {
    ...
    let tx = env.get_raw_transaction_verbose(&txid, &block_hash)?;
    ...
    let prev_tx = env.get_raw_transaction_verbose(&prev_txid, &prev_block_hash)?;
    let txout = prev_tx
        .outputs
        .iter()
